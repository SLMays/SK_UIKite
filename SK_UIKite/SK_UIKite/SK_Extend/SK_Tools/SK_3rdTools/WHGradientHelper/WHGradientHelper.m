...
//   ChromatoAnimation
+ (void)addGradientChromatoAnimation:(UIView *)view directionType:(WHGradientDirection)directionType
{
    if (view == nil) {
        return;
    }
    ...
    [chromatoLayer setColors:@[(__bridge id)UIColorFromRGBA(1, 1, 1, 0).CGColor, (__bridge id)UIColorFromRGBA(1, 1, 1, 0).CGColor]];
    chromatoLayer.locations = @[@(0.0f) ,@(1.0f)];
    [chromatoLayer setFrame:CGRectMake(0, 0, btnWidth, btnHeight)];
    
    if (directionType == WHLinearGradientDirectionLevel) {
        chromatoLayer.startPoint = CGPointMake(0, 0);
        chromatoLayer.endPoint = CGPointMake(1, 0);
    }else if (directionType == WHLinearGradientDirectionVertical){
        chromatoLayer.startPoint = CGPointMake(0, 0);
        chromatoLayer.endPoint = CGPointMake(0, 1);
    }else if (directionType == WHLinearGradientDirectionUpwardDiagonalLine){
        chromatoLayer.startPoint = CGPointMake(0, 0);
        chromatoLayer.endPoint = CGPointMake(1, 1);
    }else if (directionType == WHLinearGradientDirectionDownDiagonalLine){
        chromatoLayer.startPoint = CGPointMake(0, 1);
        chromatoLayer.endPoint = CGPointMake(1, 0);
    }
