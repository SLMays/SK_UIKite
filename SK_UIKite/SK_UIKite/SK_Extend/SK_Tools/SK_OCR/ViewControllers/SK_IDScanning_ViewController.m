//
//  SK_IDScanning_ViewController.m
//  SK_UIKite
//
...
@interface SK_IDScanning_ViewController ()
@property (strong, nonatomic) XLScanResultModel * infoModel;//身份证信息
@property (nonatomic, strong) XLScanManager *cameraManager;//相机
@property (nonatomic, strong) IDOverLayerView *BankOverlayView;//扫描动画
@end

@implementation SK_IDScanning_ViewController
...
-(IDOverLayerView *)BankOverlayView {
    if(!_BankOverlayView) {
        CGRect rect = [IDOverLayerView getOverlayFrame:[UIScreen mainScreen].bounds];
        _BankOverlayView = [[IDOverLayerView alloc] initWithFrame:rect];
